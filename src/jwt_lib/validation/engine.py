...
from jwt_lib.validation.rules import ClaimRule
...
class ClaimValidator:
    ...
    def __init__(self, rules: Iterable[ClaimRule] | None = None) -> None:
        ...
        logger.debug(f"Initialized ClaimValidator with {len(self._rules)} rule(s)")
    ...
    def add_rule(self, rule: ClaimRule) -> "ClaimValidator":
        ...
        logger.debug(f"Added claim rule {rule.__class__.__name__}")
        return self
    ...
    async def validate(self, claims: TrustedClaims) -> None:
        ...
        """
        logger.debug(f"Executing {len(self._rules)} claim rule(s).")
