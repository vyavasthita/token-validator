...
from jwt_lib.validation import ClaimValidator, RequireScopes, RequireGrantType, RequireClaim
...
class TestClaimValidator:
    ...
    @pytest.mark.asyncio
    async def test_validate_passes_all_rules(self):
        ...
        # Should not raise
        await validator.validate(claims)

    @pytest.mark.asyncio
    async def test_validate_fails_on_first_failing_rule(self):
    ...
    @pytest.mark.asyncio
    async def test_validate_with_empty_rules(self):
    ...
    @pytest.mark.asyncio
    async def test_add_rule_method(self):
        ...
        # Should pass
        await validator.validate(claims_with_scope)
        # Should fail
        with pytest.raises(PermissionDeniedError):
            await validator.validate(claims_without_scope)
...
class TestClaimValidatorIntegration:
    ...
    @pytest.mark.asyncio
    async def test_api_access_validation(self):
        ...
        # Should pass
        await validator.validate(valid_claims)

    @pytest.mark.asyncio
    async def test_admin_access_validation(self):
        ...
        # Should pass
        await validator.validate(admin_claims)
        ...
        with pytest.raises((PermissionDeniedError, InvalidClaimError)):
            await validator.validate(user_claims)
