...
from jwt_lib.profiles import (
    UserProfile,
    Auth0Profile,
)
from jwt_lib.exceptions import InvalidClaimError, PermissionDeniedError
from jwt_lib.validation.rules import RequireClaim, RequireScopes
...
class TestUserProfile:
    ...
    @pytest.mark.asyncio
    async def test_valid_claims_pass(self, profile, valid_user_claims):
        """Test that valid claims pass validation."""
        await profile.validate(valid_user_claims)  # Should not raise

    @pytest.mark.asyncio
    async def test_wrong_token_type_fails(self, profile, valid_user_claims):
        ...
        with pytest.raises(InvalidClaimError, match="tokenType"):
            await profile.validate(claims)

    @pytest.mark.asyncio
    async def test_wrong_principal_type_fails(self, profile, valid_user_claims):
    ...
    @pytest.mark.asyncio
    async def test_invalid_connection_method_fails(self, profile, valid_user_claims):
    ...
    @pytest.mark.asyncio
    async def test_invalid_audience_fails(self, profile, valid_user_claims):
    ...
    @pytest.mark.asyncio
    async def test_invalid_issuer_fails(self, profile, valid_user_claims):
    ...
    @pytest.mark.asyncio
    async def test_extra_rules_can_fail(self, profile, valid_user_claims):
    ...
    @pytest.mark.asyncio
    async def test_extra_rules_can_pass(self, profile, valid_user_claims):
    ...
    @pytest.mark.asyncio
    async def test_extra_rules_reject_non_claim_rule(self, profile, valid_user_claims):
        """Extra rules must inherit from ClaimRule for safety."""

        class NotARule:
            pass

        with pytest.raises(InvalidClaimError, match="Extra rules must inherit"):
            await profile.validate(valid_user_claims, extra_rules=[NotARule()])
...
class TestAuth0Profile:
    ...
    @pytest.mark.asyncio
    async def test_profile_validates_claims(self):
    ...
    @pytest.mark.asyncio
    async def test_profile_rejects_missing_scope(self):
    ...
    @pytest.mark.asyncio
    async def test_profile_rejects_wrong_app_name(self):
    ...
    @pytest.mark.asyncio
    async def test_profile_rejects_wrong_grant_type(self):
    ...
    @pytest.mark.asyncio
    @pytest.mark.asyncio
    async def test_profile_rejects_wrong_audience_value(self):
        """Mismatched audiences should raise InvalidClaimError."""
        claims = TrustedClaims({
            "iss": "https://auth.example.test/",
            "aud": "https://api.other.test",
            "sub": "client@clients",
            "exp": 9999999999,
            "scope": "read:models",
            "gty": "client-credentials",
    ...
    @pytest.mark.asyncio
    async def test_profile_rejects_non_string_audience(self):
        """Non-string claims should fail the equality check."""
        claims = TrustedClaims({
            "iss": "https://auth.example.test/",
            "aud": ["https://api.example.test"],
            "sub": "client@clients",
